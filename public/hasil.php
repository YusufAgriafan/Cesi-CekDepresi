...
if ($hasil <= 0.15){
  $result = mysqli_query($connect, "SELECT teks1, teks2 FROM teks WHERE no_teks = '1'");
} elseif ($hasil > 0.15 && $hasil < 0.25){
  $membershipLow = (0.25 - $hasil) / (0.25 - 0.15);
  $membershipMediumLow = ($hasil - 0.15) / (0.25 - 0.15);
  $maxMembership = max($membershipLow, $membershipMediumLow);
} elseif ($hasil >= 0.25 && $hasil <= 0.35){
  $result = mysqli_query($connect, "SELECT teks1, teks2 FROM teks WHERE no_teks = '2'");
} elseif ($hasil > 0.35 && $hasil < 0.45){
  $membershipMediumLow = (0.45 - $hasil) / (0.45 - 0.35);
  $membershipMedium = ($hasil - 0.35) / (0.45 - 0.35);
  $maxMembership = max($membershipMediumLow, $membershipMedium);
} elseif ($hasil >= 0.45 && $hasil <= 0.55){
  $result = mysqli_query($connect, "SELECT teks1, teks2 FROM teks WHERE no_teks = '3'");
} elseif ($hasil > 0.55 && $hasil < 0.65){
  $membershipMedium = (0.65 - $hasil) / (0.65 - 0.55);
  $membershipMediumHigh = ($hasil - 0.55) / (0.65 - 0.55);
  $maxMembership = max($membershipMedium, $membershipMediumHigh);
} elseif ($hasil >= 0.55 && $hasil <= 0.75){
  $result = mysqli_query($connect, "SELECT teks1, teks2 FROM teks WHERE no_teks = '4'");
} elseif ($hasil > 0.75 && $hasil < 0.85){
  $membershipMediumHigh = (0.85 - $hasil) / (0.85 - 0.75);
  $membershipHigh = ($hasil - 0.75) / (0.85 - 0.75);
  $maxMembership = max($membershipMediumHigh, $membershipHigh);
} elseif ($hasil >= 0.85){
  $result = mysqli_query($connect, "SELECT teks1, teks2 FROM teks WHERE no_teks = '5'");
}

// Menentukan kategori teks berdasarkan nilai keanggotaan tertinggi
if (isset($membershipLow) && $maxMembership == $membershipLow) {
  $teksCategory = 1;
} elseif (isset($membershipMediumLow) && $maxMembership == $membershipMediumLow) {
  $teksCategory = 2;
} elseif (isset($membershipMedium) && $maxMembership == $membershipMedium) {
  $teksCategory = 3;
} elseif (isset($membershipMediumHigh) && $maxMembership == $membershipMediumHigh) {
  $teksCategory = 4;
} elseif (isset($membershipHigh) && $maxMembership == $membershipHigh) {
  $teksCategory = 5;
}
...
mysqli_query($connect, "INSERT INTO hasil (jenisk, umur, pendidikan, menikah, hasil, nama, wa, email) VALUES 
($jenisk, $umur, $pendidikan, $menikah, '$hasil', '$nama', '$wa', '$email')");
?>
